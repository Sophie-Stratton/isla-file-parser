export class Patient {
  ...
  addName(fullName: string) {
    ...
    if (numberNames < 2) {
      console.log(
        `Unexpected names length. Got ${numberNames} names (from full name ${fullName}), expected 2 or 3.`,
      );
    ...
    if (numberNames > 3) {
      ...
      console.log(
        "Continuing with the first 3 names (last name, first name and middle name), but there may be errors.",
      );
    }
