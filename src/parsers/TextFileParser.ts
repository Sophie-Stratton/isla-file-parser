...
export class TextFileParser {
      ...
      [
        {
          fieldName: "fullName",
          fieldPosition: 4,
          parsingFunction: TextFileParser.parseName,
        },
        {
          fieldName: "dateOfBirth",
          fieldPosition: 8,
          parsingFunction: TextFileParser.parseDateOfBirth,
        },
      ],
    ],
    [
      ...
      [
        {
          fieldName: "diagnosis",
          fieldPosition: 4,
          parsingFunction: TextFileParser.parseDiagnosis,
        },
      ],
    ],
  ]);
  ...
  processContents(data: Buffer): Patient {
    ...
    // Ideally we should be seeing if this Patient is already recorded in the database, and if so update that record rather than creating a new one.
    ...
    for (const segmentToExtract of TextFileParser.FIELDS_TO_EXTRACT.keys()) {
      ...
      const relevantSegments = segments.filter((line) =>
        line.startsWith(segmentToExtract),
      );
      if (relevantSegments.length != 1) {
        console.log(
          `Found ${relevantSegments.length} segments for field ${segmentToExtract}. Stopping processing of file ${this.filename}`,
        );
      ...
      for (const fieldToExtract of TextFileParser.FIELDS_TO_EXTRACT.get(
        segmentToExtract,
      )) {
  ...
  static parseDateOfBirth(dateOfBirth: string, patient: Patient) {
    if (dateOfBirth.length !== 8) {
      console.log(
        `Got unexpected date format (string was of length ${dateOfBirth.length})`,
      );
      console.log("Continuing, without setting date of birth.");
    } else {
      const date = new Date(
        `${dateOfBirth.slice(0, 4)}-${dateOfBirth.slice(4, 6)}-${dateOfBirth.slice(6)}`,
      );
